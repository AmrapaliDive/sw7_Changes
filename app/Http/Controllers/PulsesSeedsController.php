...
class PulsesSeedsController extends Controller
{
    public function getIndex(){
        $searchNutInput = "dryfruits";
    ...
    public function getChana(){
        $searchNutInput = "chana";
        $products = Product::where( 'type', 'LIKE', '%'.$searchNutInput.'%')->orWhere ('subtype', 'LIKE', '%' .$searchNutInput. '%')->orWhere ('title', 'LIKE', '%' .  $searchNutInput . '%')->paginate();		
       // $products = DB::table('products')->orderBy('id', 'DESC')->paginate(12);
        return view('shop.pulses-seeds',['products' => $products]);
    }

    public function getPeans(){
        $searchNutInput = "peans";
        $products = Product::where( 'type', 'LIKE', '%' .  $searchNutInput . '%')->orWhere ('subtype', 'LIKE', '%' .$searchNutInput. '%')->orWhere ('title', 'LIKE', '%' .  $searchNutInput . '%')->paginate();		
       // $products = DB::table('products')->orderBy('id', 'DESC')->paginate(12);
        return view('shop.pulses-seeds',['products' => $products]);
    }
  
    public function getMung(){
        $searchNutInput = "mung";
        $products = Product::where( 'type', 'LIKE', '%' .  $searchNutInput . '%')->orWhere ('subtype', 'LIKE', '%' .$searchNutInput. '%')->orWhere ('title', 'LIKE', '%' .  $searchNutInput . '%')->paginate();		
       // $products = DB::table('products')->orderBy('id', 'DESC')->paginate(12);
        return view('shop.pulses-seeds',['products' => $products]);
    ...
    public function postCheckout(Request $request) {
        ...
        return redirect()->route('product.dryfruits')->with('success','Successfully purchased products!');
    }
