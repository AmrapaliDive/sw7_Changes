...
<div class="container col-md-6 offset-md-4" style="padding-top:10px;margin-left:350px;">
	...
	<!-- <h3 class="" style="font-size:30px;margin-top:20px;"><center><strong>Admissions</strong> </center></h3><hr style=""> -->
		...
		<form style="" action="{{url('/admission/records')}}" method="POST">
		{{ csrf_field() }}
	...
	<form style="" action="{{url('/admission/recordsByFranchisee')}}" method="POST">
		{{ csrf_field() }}
	<div class="col-md-4" style="margin-top:30px;">
            <div class="form-group">
                <label for="name" class=" " style="color:black ;">&nbsp;<i class="fa fa-user"></i>&nbsp;Choose Franchisee</label>
				<select id="franchisee" name="franchisee" class="form-control">
				@foreach($franchisees as $franchisee)
				<option value="{{$franchisee->id}}">{{$franchisee->first_name}} {{$franchisee->last_name}}</option>
				@endforeach
				</select>
            </div>
            </div>
...
<!-- <div class="panel panel-default container" style="overflow:auto;padding-top:30px;"> -->
	...
	<a class="btn btn-primary btn-lg pull-left "style="float:right;width:200px;margin:10px;margin-top:25px;font-size:1.4rem" href="{{ url('/admin/admissions') }}">Back</a>
	</div>
